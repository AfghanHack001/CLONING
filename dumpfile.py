...
def menu(my_name,my_id):
	...
	banner()
	sg = '# THIS SCRIPT SHOULD ONLY BE USED BY PEOPLE WHO HAVE OBTAINED PERMISSION FROM AUTHOR : Reddy, CONTACT ACCESS PERMIT:085718094505'
	fx = mark(sg, style=' bold blue')
	sol().print(fx)
	sg = '# 🖥️SCRIPT USER NAME🖥️ '
	fx = mark(sg, style=' bold green')
	sol().print(fx)
	print(x+'['+h+'✓'+x+'] NAME   Reddy : '+str(my_name))
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/Reddy-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
		...
		try:vin = os.listdir('/sdcard/Reddy-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/Reddy-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/Reddy-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/Reddy-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/Reddy-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
